              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-rule">
                    <th className="text-left py-2 pr-4 text-xs font-medium text-ink-soft cursor-pointer hover:text-ink select-none whitespace-nowrap" onClick={() => toggleSort('date')}>
                      Date <SortIcon field="date" />
                    </th>
                    <th className="text-left py-2 pr-4 text-xs font-medium text-ink-soft cursor-pointer hover:text-ink select-none w-full" onClick={() => toggleSort('description')}>
                      Description <SortIcon field="description" />
                    </th>
                    <th className="text-right py-2 pr-4 text-xs font-medium text-ink-soft cursor-pointer hover:text-ink select-none whitespace-nowrap" onClick={() => toggleSort('amount')}>
                  ...
                  {sorted.map((exp) =>
                    editingId === exp.id ? (
                      ...
                      <tr key={exp.id} className="border-b border-ledgerbar hover:bg-ledgerbar group">
                        <td className="py-2.5 pr-4 text-ink-soft whitespace-nowrap text-xs">{formatShortDate(exp.date)}</td>
                        ...
                        <td className="py-2.5">
                          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={() => startEdit(exp)} title="Edit"
                              className="w-6 h-6 flex items-center justify-center rounded text-ink-soft hover:text-ink hover:bg-ledgerbar transition-colors">
                              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                              </svg>
                            </button>
                            <button onClick={() => setDeleteTargetId(exp.id!)} title="Delete"
                              className="w-6 h-6 flex items-center justify-center rounded text-ink-soft hover:text-debit hover:bg-red-50 transition-colors">
                              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <tfoot>
                  <tr className="border-t border-rule">
                    <td colSpan={2} className="pt-3 text-xs font-medium text-ink-soft">
