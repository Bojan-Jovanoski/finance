                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-slate-100">
                      <th
                        className="text-left py-2 pr-4 text-xs font-medium text-slate-500 cursor-pointer hover:text-slate-800 select-none whitespace-nowrap"
                        onClick={() => toggleSort('date')}
                      >
                        Date <SortIcon field="date" />
                      </th>
                      <th
                        className="text-left py-2 pr-4 text-xs font-medium text-slate-500 cursor-pointer hover:text-slate-800 select-none w-full"
                        onClick={() => toggleSort('description')}
                      >
                        Description <SortIcon field="description" />
                      </th>
                      <th
                        className="text-right py-2 pr-4 text-xs font-medium text-slate-500 cursor-pointer hover:text-slate-800 select-none whitespace-nowrap"
                    ...
                    {sorted.map((exp) =>
                      editingId === exp.id ? (
                        ...
                        <tr key={exp.id} className="border-b border-slate-50 hover:bg-slate-50 group">
                          ...
                          <td className="py-2.5 pr-4 text-right font-medium text-slate-800 whitespace-nowrap">
                  ...
                  <tfoot>
                    <tr className="border-t border-slate-200">
                      ...
                      <td className="pt-3 text-right font-semibold text-slate-900 whitespace-nowrap">
                        {formatMKD(total)}
                      </td>
                      <td />
                    </tr>
    ...
    <tr className="border-b border-indigo-100 bg-indigo-50/50">
      ...
      <td className="py-1.5">
        <div className="flex items-center gap-1">
          <button
            onClick={onSave}
            className="w-6 h-6 flex items-center justify-center rounded text-emerald-600 hover:bg-emerald-50 transition-colors"
            title="Save"
          >
            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
