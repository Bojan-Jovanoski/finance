...
export function MonthSetup({ month }: MonthSetupProps) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const sav = parseFloat(savingsGoal.replace(/[.,]/g, ''));
    if (!income || isNaN(inc) || inc < 0) {
      setError('Enter a valid income amount.');
      return;
    }
    if (!savingsGoal || isNaN(sav) || sav < 0) {
      setError('Enter a valid savings goal.');
      return;
    }
    if (sav > inc) {
      setError('Savings goal cannot exceed income.');
      return;
    }
    setError('');
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 w-full max-w-md">
        <div className="mb-6">
          ...
          <p className="text-sm text-slate-500 mt-1">
            Enter your income and savings goal to get started.
          </p>
        </div>
