    ...
    <>
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <Card label="Income" value={formatMKD(budget.income)} color="slate" editing={editing}
            editContent={<input type="number" min="0" step="1" value={income} onChange={(e) => setIncome(e.target.value)}
          ...
          <Card label="Savings goal" value={formatMKD(budget.savingsGoal)} color="indigo" editing={editing}
            editContent={<input type="number" min="0" step="1" value={savingsGoal} onChange={(e) => setSavingsGoal(e.target.value)}
              className="w-full text-sm font-semibold bg-transparent border-b border-indigo-300 focus:outline-none focus:border-indigo-600 pb-0.5" />} />
          <Card label="Total spent" value={formatMKD(totalSpent)}
            sub={spendable > 0 ? `${Math.min(100, Math.round((totalSpent / spendable) * 100))}% of budget` : undefined}
            color={isOverBudget ? 'red' : 'slate'} />
          <Card label="Remaining" value={formatMKD(Math.abs(remaining))}
        ...
        <div className="flex items-center gap-2">
          {editing ? (
            <form onSubmit={handleSave} className="flex items-center gap-2">
              <button type="submit" className="text-xs font-medium text-indigo-700 hover:text-indigo-900 transition-colors">Save changes</button>
              <span className="text-slate-300">·</span>
              <button type="button" onClick={handleCancelEdit} className="text-xs text-slate-500 hover:text-slate-700 transition-colors">Cancel</button>
              {error && <span className="text-xs text-red-500 ml-2">{error}</span>}
            </form>
          ) : (
            <button onClick={() => { setIncome(String(budget.income)); setSavingsGoal(String(budget.savingsGoal)); setEditing(true); }}
      ...
      {confirmDiscard && (
        <ConfirmDialog message="Discard unsaved changes to income and savings goal?"
...
function Card({ label, value, sub, color, editing, editContent }: {
  ...
  const accent: Record<CardColor, string> = {
    slate: 'text-slate-900', indigo: 'text-indigo-700', red: 'text-red-600',
